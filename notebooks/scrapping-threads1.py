...
def save_to_csv(posts: List[Dict], search_term: str):
    ...
    try:
        ...
        filename = f"threads_{search_term.replace(' ', '_')}_{timestamp}.csv"
        ...
        if os.path.exists(output_path):
            logger.info(f"Successfully verified file save: {filename}")
            return df
        else:
...
if __name__ == "__main__":
    try:
        ...
        logger.info(f"Searching for posts from {cutoff_time} to {now}")
        ...
        df = save_to_csv(posts, search_term)
        print(f"\nCollected {len(posts)} posts within the last {days_back} days")
        if df is not None:
            print("\nLatest posts:")
            print(df.head())
